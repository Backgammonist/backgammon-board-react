...
export const Sidebar: React.FC = () => {
    ...
    const { boardWidth, borderWidth, sidebarWidth, pointWidth, checkerWidth, boardHeight, checkerStroke, cubeWidth } = useDimensions();
    ...
    return (<svg key="sidebar" width={sidebarWidth * (boardWidth - sidebarWidth) / boardWidth} height={boardHeight * (boardWidth - sidebarWidth) / boardWidth} viewBox={`0 0 ${sidebarWidth} ${boardHeight}`} xmlns="http://www.w3.org/2000/svg">
        ...
        {doublingCube?.value && <DoublingCube value={doublingCube?.value ?? 2} owner={doublingCube?.owner ?? null} />}

        {[...new Array(opponentOut)].map((_, i) => (<rect key={`opponent-out-${i}`} x={xPos} y={i * gap + borderWidth + cubeWidth} width={width} height={height} rx={3} ry={3} fill={opponentCheckerColor} />))}
        <g transform={`scale(1 -1) translate(0, -${boardHeight})`}>
            {[...new Array(playerOut)].map((_, i) => (<rect key={`player-out-${i}`} x={xPos} y={i * gap + borderWidth + cubeWidth} width={width} height={height} rx={3} ry={3} fill={playerCheckerColor} />))}
        </g>
    </svg>);
};